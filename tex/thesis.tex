...
This binary sits inside the container but is invoked on the host in the root context of the container from the perspective of the kernel. \cite{CVE-2022-0811}
...
access to the Kubernetes control plane and the Kubernetes network. The foundation of severe attacks is being able to move and operate from inside the cluster or a container. 
If a malicious actor did not have the ability to freely create files in any desired directory of a container, CVEs such as CVE-2019-5736 and CVE-2022-29179 become difficult to exploit.
Missing write permissions to sensitive directories would be a major obstacle for performing a container escape, and missing Kubernetes API permissions would decrease 
zn attackers options to query for sensitive data such as secrets. 


\chapter{Hardening Measures}
Missing write permissions to sensitive directories are a major obstacle for performing container escapes, and missing permissions for accessing the Kubernetes API
would limit an attackers options to query for sensitive cluster data in case of compromise. Strict network settings prevent attackers from installing malicious software packages
...
data and permissions to perform actions required to complete their task.  
...
of root deployments in the first place.  
\newline
\newline
...
should be externalized from the source code and repository platforms like GitHub. Major cloud providers like AWS and Azure provide 
...
accurate and timely vulnerability reports. 
\newline
\newline
Runtime security solutions allow for monitoring and blocking certain kernel events or suspicious system calls. 
Tools like Falco and Tetragon help gain visibility into abnormal behavior of hosts and containers. 
...
attack surface. The thesis highlights the importance of a clear security initiative when utilizing Kubernetes in 
