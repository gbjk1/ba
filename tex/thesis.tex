...
\\\\\\
Date: 14.01.2024 \hspace{6cm} Signature:\\
...
A pod is a unit of one or more containers, which belong together and share resources such as network 
...
The rules are enforced by the Kubernetes network plugin, ensuring that only authorized traffic is allowed to pass between pods. \cite{networkpolicy}
...
also include specification changes or even specification deprecations (e.g., removal of affected protocols or functionality in their entirety). \cite{nist}"
\\
In addition, the Common Vulnerability Scoring System (CVSS) "is a method used to supply a qualitative measure of severity" \cite{nist}. 
...
"libc.so" as soon as a developer runs "rkt-enter", which is the default /bin/bash command, to create a new shell. This sets off malicious 
...
As a result, the adversary is able to read and write to the host filesystem. \cite{purdue} \cite{CVE-2019-101} 
...
CVE-2022-23648 is another vulnerability related to container-escape, this time found in containerd, a popular Kubernetes runtime. The vulnerability is due to a vulnerable part of Containerd's CRI plugin that deals with volumes. 
It can be taken advantage of by an attacker by including a volume that contains path traversal in the image. This enables copying files from the host to a mounted path inside a container. More precisely, 
the "copyExistingContents" function in Containerd's code is where the vulnerability is located. This feature transfers the files from the volume path under the control of 
the attacker to a temporary folder that is subsequently mounted inside a container. Path traversal can be used by an attacker to deceive this method into copying any files 
from the host filesystem. This may result in private information being revealed.
This can lead to the disclosure of confidential information. The severity of this vulnerability is rated as high, with a CVSS base score of 7.5. \cite{crowd} \cite{CVE-2022-23648}
...
without any limits or throttling, which can lead to uncontrolled resource consumption. The official CVE description of NIST states:
\newline
\newline
 "The ExecSync request runs commands in a container and logs the output of the command. This output is then read by CRI-O after command execution, and it is read in a manner where the entire file 
corresponding to the output of the command is read in." \cite{nist2}
\newline It is thus possible to drain memory or disk space of the machine when CRI-O reads an extensive output. 
...
Flux2 is a continuous deployment tool which periodically aligns the state of a cluster with the state defined in a remote repository. 
As reported in CVE-2022-24817, when provided with a kubeconfig file with the correct access rights, such files can be used to define commands to be executed.
A perpatrator might create a kubeconfig file to run any code inside the controller's container if they had write access to a flux source or direct access 
to the target cluster. If the service account of the controller has high permissions, this can also result in privilege escalation in multi-tenancy setups.
To exploit the issue, one of the following permission sets would need to be established within the range of affected versions: 
\begin{itemize}
  \item Direct access to the cluster for Kubernetes Secrets and Flux Kustomization
  \item Direct access to the cluster to change the Kubernetes secrets that are currently being utilised in Flux Kustomization.
  \item Direct access to the cluster allows for the creation or modification of existing Kubernetes secrets as well as Flux Kustomization. 
  \item Authorization to alter a Flux Source's configuration. 
\end{itemize}
\cite{flux2}
The vulnerability is marked with a CVSS base score of 9.9. \cite{CVE-2022-24817}
...
This ensures that the container is started with a non-root user ID rather than the normal root UID of 0. 
Whenever possible, it is advised to run containers as non-root to limit the possibilities to create privilege escalation or escape exploits.  \cite{seccontext}
\subsubsection{runAsUser}
This setting in Kubernetes declares the user ID that will be used to run the container.  \cite{seccontext}
\subsubsection{allowPrivilegeEscalation}
The ability to escalate a container's privileges is determined by this setting. When true, it gives a container access to extra privileges not available by default.
When set to false, obtaining higher privileges, by 'sudo' or 'setuid' operations for example, 
