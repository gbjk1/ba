...
To exemplify this definition, picture an enterprise Java application. The container of such an application
would specify the underlying Host-OS, the JDK, which includes the JVM and other Java binaries, an application 
...
Containers run as isolated environments, separate from the host system and other containers. 
This isolation of environments is mainly achieved by the namespace and cgroup technology of the Linux kernel.

The kernel:
The Linux kernel is the core component of the Linux operating system. It acts as a communication bridge between a user's 
applications and the underlying hardware. It is responsible for managing the fundamental functionality of the Linux system, from 
scheduling processes to providing resources to applications.

The kernel can be modified according to user needs and provides a wide range of features such as memory management, 
file system management, process management, device drive management, and network support. For example, the actual low level 
implementation of the TCP protocol is handled by the kernel.  

Piecing together the functionality of 
